...
public:
	...
	Billboard(HWND hWnd, WCHAR textura[])
	{
		Up = VectorRR(0, 1, 0);
	...
	void Draw(GLdouble PX, GLdouble PY, GLdouble PZ, float escX, float escY, float alt, float posX, float posY, float posZ)
	{
		ancho = escX;
		alto = escY;
		x = posX;
		...
		BillCam = VectorRR(PX - posX, PY - posY, PZ - posZ);
		...
		glBegin(GL_QUADS);
		glTexCoord2f(0, 0);
		glVertex3f(posX - derecha.X, posY + escY, posZ - derecha.Z);

		glTexCoord2f(1, 0);
		glVertex3f(posX + derecha.X, posY + escY, posZ + derecha.Z);

		glTexCoord2f(1, 1);
		glVertex3f(posX + derecha.X, posY - alt / 2 , z + derecha.Z);
